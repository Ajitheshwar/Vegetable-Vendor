...
const getCategoriesAndSubCategories = async (req, res) => {
  try {
    ...
    res.status(200).send({data : categoriesAndSubCategories});
  } catch (error) {
    console.log(error);
    res
      .status(400)
  ...
  try {
    ...
    if (category == "all") {
      Promise.all([
        ...
        .catch((error) => {
          throw new Error("Error in getting Category Filter Data", error);
        });
    } else {
      Promise.all([
    ...
    res
      .status(400)
      .send("Error in getting Category Data\nPlease refresh your page");
  }
  //console.log(result)
};
...
const getProductsByFilter = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .send("Error in applying filtering\nPlease refersh your page");
  }
};
